...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_check_overwrite_tile(self):
        '''Checks if one user places a tile on a block, than the second user cannot place a tile on that same block.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        tiles[0].click() #Clicks again after having changed turns
        self.assertTileIs(tiles[0], self.SYMBOL_X)

    def test_cant_place_after_win(self):
        '''Ensures adittional pieces cant be placed after a player has won.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click() #X plays 0
        tiles[3].click() #O
        tiles[1].click() #X
        tiles[4].click() #O
        tiles[2].click() #X wins!
        
        tiles[8].click() #Should not work, as X has won
        self.assertTileIs(tiles[8], self.SYMBOL_BLANK)

    def test_can_move_piece(self):
        '''Ensures pieces can be moved.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click() #X
        tiles[5].click() #O
        tiles[1].click() #X
        ...
        tiles[3].click() #X
        tiles[8].click() #O
        #Nobody has won, time for x to move!
        tiles[3].click() #X chooses to move tile 3
        tiles[6].click() #X moves to 6
        #Assert tiles are properly emptied
